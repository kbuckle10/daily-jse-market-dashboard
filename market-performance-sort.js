(() => {
  const data=()=>window.JSE_DASHBOARD_DATA?.stocks||[];
  const byTicker=()=>new Map(data().map(s=>[String(s.ticker||'').toUpperCase(),s]));
  const num=v=>v==null||v===''||!Number.isFinite(Number(v))?null:Number(v);
  const ticker=s=>String(s?.ticker||'');
  const tie=(a,b)=>ticker(a).localeCompare(ticker(b));
  const asc=(field,a,b)=>{const av=num(a?.[field]),bv=num(b?.[field]);if(av==null&&bv==null)return tie(a,b);if(av==null)return 1;if(bv==null)return -1;return av-bv||tie(a,b);};
  const desc=(field,a,b)=>{const av=num(a?.[field]),bv=num(b?.[field]);if(av==null&&bv==null)return tie(a,b);if(av==null)return 1;if(bv==null)return -1;return bv-av||tie(a,b);};
  const date=v=>{if(!v)return null;const d=new Date(v);return Number.isFinite(d.valueOf())?d.valueOf():null;};
  const event=s=>{const base={exDate:s?.exDate,recordDate:s?.recordDate,payDate:s?.payDate};const sa=s?.saLatestDividend;const ev=x=>Math.max(date(x?.exDate)||0,date(x?.recordDate)||0,date(x?.payDate)||0);const cross=s?.primaryListing?.market&&String(s.primaryListing.market).toUpperCase()!=='JMSE';return !cross&&sa&&ev(sa)>ev(base)?sa:base;};
  ...
  const opportunity=s=>Math.max(0,Math.min(120,(num(s?.score)??50)+(s?.zoneStatus==='below'?12:s?.zoneStatus==='in'?7:s?.zoneStatus==='above'?-10:0)));
