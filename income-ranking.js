(()=>{
  ...
  function rerank(){
    ...
    rows.sort((a,b)=>{
      ...
      const ay=currentYield(sa),by=currentYield(sb);
      if(ay==null&&by==null)return String(sa.ticker).localeCompare(String(sb.ticker));
      if(ay==null)return 1;if(by==null)return-1;
      return by-ay||String(sa.ticker).localeCompare(String(sb.ticker));
    });
    rows.forEach(r=>list.appendChild(r));
    list.dataset.incomeRank='current-yield-desc';
    const leader=document.getElementById('incomeLeader'),s=map.get(tickerFromRow(rows[0]));
    if(leader&&s){const y=currentYield(s);leader.textContent=y==null?'No current yield available':`${s.ticker}: ${y.toFixed(2)}% current yield`;}
  }
