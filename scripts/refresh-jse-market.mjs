...
function headerIndex(headers, patterns) {
  ...
  for (const re of patterns) {
    const i = lowered.findIndex(h => re.test(h));
    if (i >= 0) return i;
  }
  return -1;
}

function jamaicaTodayLabel() {
  return new Intl.DateTimeFormat('en-US', {
    timeZone: 'America/Jamaica', month: 'short', day: 'numeric', year: 'numeric'
  }).format(new Date());
}
...
async function gotoJse(page) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      console.warn(`JSE page did not expose usable content on attempt ${attempt}.`);
    } catch (err) {
      console.warn(`JSE navigation attempt ${attempt} failed: ${err.message}`);
    }
...
async function extractTables(page) {
  return page.locator('table').evaluateAll(tables => tables.map(table => {
    ...
    if (!effectiveHeaders.length) {
      ...
      effectiveHeaders = first ? Array.from(first.querySelectorAll('th,td')).map(x => (x.textContent || '').trim()) : [];
    }
...
function scoreTable(table, tickers) {
  ...
  const headerScore = /symbol|ticker|security/.test(headerText) ? 2 : 0;
  const priceScore = /close|closing|last/.test(headerText) ? 2 : 0;
  return tickerHits * 10 + headerScore + priceScore;
}
...
function parseTable(table, tickers) {
  ...
  for (const row of table.rows) {
    ...
    if (price == null) {
      const symbolCell = upper.findIndex(cell => cell === ticker);
      const numericAfter = row.slice(symbolCell + 1).map(num).filter(v => v != null);
      if (numericAfter.length) price = numericAfter[numericAfter.length - 1];
    }
    if (!(price > 0)) continue;
...
let tradeDate = jamaicaTodayLabel();
try {
  if (await gotoJse(page)) {
    const body = await page.locator('body').innerText().catch(() => '');
    tradeDate = extractTradeDate(body);
    const tables = await extractTables(page);
    console.log(`JSE rendered ${tables.length} table(s); detected trade date ${tradeDate}.`);
    if (tables.length) {
      const ranked = [...tables].sort((a, b) => scoreTable(b, tickers) - scoreTable(a, tickers));
      for (const table of ranked) {
        const parsed = parseTable(table, tickers);
        for (const [ticker, quote] of parsed) if (!captured.has(ticker)) captured.set(ticker, quote);
        if (captured.size === tickers.length) break;
      }
    }
    if (!captured.size) {
      console.warn('No tracked tickers were parsed from JSE. Body sample follows:');
...
for (const stock of data.stocks) {
  const quote = captured.get(String(stock.ticker).toUpperCase());
  if (!quote) continue;
  ...
  updated++;
  console.log(`${stock.ticker}: J$${stock.price}${stock.dayPct != null ? ` (${stock.dayPct}%)` : ''} from JSE`);
}

if (updated) {
  data.updated = tradeDate.replace(/^([A-Z][a-z]{2})/, m => ({Jan:'January',Feb:'February',Mar:'March',Apr:'April',May:'May',Jun:'June',Jul:'July',Aug:'August',Sep:'September',Oct:'October',Nov:'November',Dec:'December'}[m] || m));
  data.priceLabel = 'JSE official close where available / StockAnalysis delayed fallback';
  writeDashboard(data);
  console.log(`SUCCESS: JSE updated ${updated}/${tickers.length} tracked stocks; SA remains fallback for uncaptured quotes.`);
} else {
  console.warn(`WARNING: JSE updated 0/${tickers.length} tracked stocks; preserving StockAnalysis fallback values.`);
}
