(()=>{
  ...
  function render(){
    ...
    rank.innerHTML=ranked.map((s,i)=>`<div class="rank-row"><div class="rank-left"><span class="rank-number">${i+1}</span><div><strong>${s.ticker}</strong><div class="rank-reason">${mode==='income'?`Income ${incomeLabel(s)} • ${currentYield(s).toFixed(2)}% current yield • payout ${num(s.payoutRatio)!=null?Number(s.payoutRatio).toFixed(0)+'%':'N/A'}`:(s.reason||'')}</div><small>${mode[0].toUpperCase()+mode.slice(1)} score ${objectiveScore(s,mode).toFixed(0)}/100 • Primary ${s.rating||'N/A'}</small></div></div><span class="rating ${s.ratingClass||'hold'}">${s.rating||'N/A'}</span></div>`).join('')||'<p class="neutral">Add tickers to build your Fresh Capital ranking.</p>';
    ...
    const note=document.querySelector('#allocationSection .allocation-note');if(note)note.textContent=mode==='balanced'?'Balanced mode remains Buy-class only and combines research score, primary rating and entry zone.':'This objective may include qualifying Hold/Watch stocks; Avoid/Sell stocks remain excluded.';
