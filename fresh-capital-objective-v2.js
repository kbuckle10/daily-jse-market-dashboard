(()=>{
  ...
  function card(s,i){
    const p=pb(s),disc=num(s.bookDiscountPct),zone=s.zoneStatus==='in'?'IN ZONE':s.zoneStatus==='below'?'BELOW ZONE':s.zoneStatus==='above'?'ABOVE ZONE':'ZONE N/A';
    return `<article class="fresh-card fresh-objective-card"><div class="fresh-rank">#${i+1}</div><div class="fresh-main"><div class="fresh-title"><div><strong>${s.ticker}</strong><span>${s.company||''}</span></div><span class="rating ${s.ratingClass||'hold'}">${s.rating||'N/A'}</span></div><div class="investor-badges"><span class="sector-badge">${s.sector||'Other'}</span>${tags(s).map(x=>`<span class="style-badge ${x.toLowerCase()}">${x}</span>`).join('')}</div><div class="fresh-metrics"><span><small>Price</small><strong>J$${fmt(s.price,2)}</strong></span><span><small>Current Yield</small><strong>${fmt(currentYield(s),2)}%</strong></span><span><small>P/B</small><strong>${p==null?'N/A':fmt(p,2)+'×'}</strong></span><span><small>P/E</small><strong>${num(s.pe)==null?'N/A':fmt(s.pe,1)+'×'}</strong></span></div><div class="fresh-objective-score"><span>${mode[0].toUpperCase()+mode.slice(1)} score</span><strong>${fmt(objectiveScore(s,mode),0)}/100</strong><i><b style="width:${objectiveScore(s,mode)}%"></b></i></div><div class="fresh-footer"><p>${objectiveReason(s)}</p><span class="zone-status ${s.zoneStatus||'above'}">${zone}</span></div><div class="fresh-secondary">${disc==null?'Book value N/A':disc>=0?fmt(disc,0)+'% below book':fmt(Math.abs(disc),0)+'% above book'} <span>• Primary ${s.rating||'N/A'}</span></div></div></article>`;
  }
