(()=>{
  ...
  const canonicalRating=v=>{const s=String(v||'').trim(),k=s.toLowerCase().replace(/\s+/g,' ');if(k==='hold')return'Hold';if(k==='watch')return'Watch';if(k==='strong buy')return'Strong Buy';if(k==='buy/accumulate'||k==='buy / accumulate')return'Buy/Accumulate';if(k==='hold / wait'||k==='hold/wait')return'Hold / Wait';if(k==='avoid')return'Avoid';if(k==='sell')return'Sell';return s;};

  function ensureStyles(){if(document.getElementById('incomeDividendInsightStyles'))return;const st=document.createElement('style');st.id='incomeDividendInsightStyles';st.textContent='.income-payout-strip{display:flex;gap:8px;flex-wrap:wrap;margin:7px 0 3px}.income-payout-chip{display:flex;flex-direction:column;gap:1px;min-width:105px;padding:6px 8px;border:1px solid var(--border);border-radius:9px;background:var(--surface2)}.income-payout-chip small{font-size:.56rem;color:var(--muted);font-weight:700}.income-payout-chip strong{font-size:.72rem}.allocation-income-detail{display:block;margin-left:18px;margin-top:2px;font-size:.58rem;color:var(--muted)}.performance-rating-insight{display:block;margin-top:4px;line-height:1.35}@media(max-width:720px){#freshCapitalSection .panel-heading{align-items:flex-start;gap:10px}#freshCapitalSection .panel-heading .panel-badge{width:100%;max-width:100%;box-sizing:border-box;align-self:stretch;white-space:normal;overflow-wrap:anywhere;text-align:left}#freshCapitalObjective.fresh-objective-bar{display:block;padding:10px}#freshCapitalObjective .fresh-objective-label{display:block;margin-bottom:8px}#freshCapitalObjective .fresh-objective-buttons{display:flex;flex-wrap:nowrap;gap:6px;width:100%;min-width:0}#freshCapitalObjective .fresh-objective-btn{flex:1 1 0;min-width:0;padding:7px 4px;font-size:.58rem;text-align:center}#freshCapitalObjective .fresh-objective-note{display:block;margin-top:9px}}';document.head.appendChild(st);}

  function restructureFreshControls(){const ctl=document.getElementById('freshCapitalObjective');if(!ctl||ctl.querySelector('.fresh-objective-buttons'))return;const buttons=[...ctl.querySelectorAll('.fresh-objective-btn')];if(!buttons.length)return;const wrap=document.createElement('div');wrap.className='fresh-objective-buttons';buttons[0].before(wrap);buttons.forEach(b=>wrap.appendChild(b));}
  function normalizeFreshRatings(){document.querySelectorAll('#freshCapitalSection .rating').forEach(el=>{el.textContent=canonicalRating(el.textContent)});document.querySelectorAll('#allocationLegend .legend-item small').forEach(el=>{const txt=el.textContent||'',parts=txt.split('•').map(x=>x.trim());if(parts.length<2)return;parts[parts.length-1]=canonicalRating(parts[parts.length-1]);el.textContent='• '+parts.join(' • ');});}
  ...
  function performanceNarrative(s,includeRating=true){
    ...
    const p=positives.slice(0,3),r=risks.slice(0,2),rating=canonicalRating(s.rating||'N/A'),prefix=includeRating?`${rating}: `:'';
    if(p.length&&r.length)return `${prefix}${p.join(' • ')} support the case, but ${r.join(' and ')} warrant caution.`;
    if(p.length)return `${prefix}${p.join(' • ')} are the strongest current positives.`;
    if(r.length)return `${prefix}${r.join(' and ')} are the main current risks.`;
  ...
  function decoratePerformanceCards(){
    document.querySelectorAll('#cardView .stock-card').forEach(card=>{const s=stockForElement(card,'card'),p=card.querySelector('.rank-reason');if(s&&p)p.textContent=performanceNarrative(s,true);const r=card.querySelector('.rating');if(r)r.textContent=canonicalRating(r.textContent);card.querySelectorAll('.metric-row').forEach(row=>{const label=row.querySelector('span');if(label&&/trailing yield/i.test(label.textContent||''))label.textContent='Current yield';});});
  }
  function decoratePerformanceTable(){
    document.querySelectorAll('#stockTableBody tr').forEach(row=>{const s=stockForElement(row,'row');if(!s)return;const rating=row.querySelector('.rating');if(!rating)return;rating.textContent=canonicalRating(rating.textContent);const cell=rating.closest('td');if(!cell)return;let detail=cell.querySelector('small');if(!detail){detail=document.createElement('small');cell.appendChild(detail);}detail.classList.add('performance-rating-insight');detail.textContent=performanceNarrative(s,false);});
  }
  ...
  let timer;function refresh(delay=280){clearTimeout(timer);timer=setTimeout(()=>{ensureSortOptions();restructureFreshControls();decorateIncomeCards();decorateAllocation();applyCustomSort();decoratePerformanceCards();decoratePerformanceTable();decorateUpdatedPill();normalizeFreshRatings();},delay);}
