(() => {
  ...
  function categories(s){
    ...
    if(sc.valuation!=null&&sc.valuation>=65)cats.push('Value');
    const age=monthsSince(s.payDate),activeDividend=(age==null?Boolean(s.payDate||s.exDate||s.currentAnnualDps):age<=18);
    if((sc.yieldPct??0)>=3&&activeDividend&&sc.dividend!=null&&sc.dividend>=55)cats.push('Income');
    if(sc.growth!=null&&sc.growth>=65)cats.push('Growth');
    return cats;
  }
  window.JSE_INVESTMENT_CATEGORIES={categories,scores};
